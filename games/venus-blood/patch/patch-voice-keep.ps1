...
$targetPath = Join-Path $GameDirectory 'patch2.xp3'
$sourceSize = [int64]731761145
$targetSize = [int64]55738
$sourceHash = 'CCAB1338F4C4F32A560C5C0AF5FD3BA142C6DA5D61081CC652C537297301AD66'
$targetHash = 'F582F844536ADBC7A27E013333E0D6ACF2B7EC74DF88496585737A3B399403DD'

if (-not ('VenusBloodVoiceKeepPatch' -as [type])) {
...
public static class VenusBloodVoiceKeepPatch
{
    private static readonly byte[] Xp3Signature = new byte[] {
        0x58, 0x50, 0x33, 0x0D, 0x0A, 0x20, 0x0A, 0x1A, 0x8B, 0x67, 0x01
    };
    private const int ExpectedIndexSize = 505132;
    private const int ExpectedMacroSize = 55603;
    private const string OriginalMacroHash = "C25ABBA0B823C663928529DB0565FA4E4D0869677721689598504BB55BD7E9C4";
    private const string PatchedMacroHash = "2F91C58F3CA245F538B4186F4ECEE16664FFB78051E38EC95D5DBFE637B73B62";
    ...
    public static string Build(string sourcePath, string outputPath)
    {
        ...
        using (FileStream archive = new FileStream(sourcePath, FileMode.Open, FileAccess.Read, FileShare.Read))
        {
            ...
            byte[] index = ExpandZlib(ReadAt(archive, indexOffset + 17, storedSize), originalSize);
            if (index.Length != ExpectedIndexSize)
                throw new InvalidDataException("XP3インデックスのサイズが想定と一致しません。");

            EntryLocation location = LocateEntry(index, "system2/macro.ks");
            uint segmentFlags = ReadUInt32(index, location.SegmentFlags);
            long segmentOffset = CheckedLong(ReadUInt64(index, location.SegmentOffset), "macro.ksの位置");
        ...
        byte[] context = Encoding.ASCII.GetBytes(
            "[macro name=cm2]\r\n\t[stopse]\r\n\t[hr]\r\n\t[hr]\r\n");
        byte[] beforeStop = Encoding.ASCII.GetBytes("[macro name=cm2]\r\n");
        ...
        byte[] output = BuildArchive(patchedMacro);
...
function Get-SourceState {
    ...
    if ($file.Length -eq $sourceSize -and (Get-Sha256 $sourcePath) -eq $sourceHash) {
        return 'Supported'
    }
    return 'Unsupported'
}
...
if (Get-Process -Name 'blood' -ErrorAction SilentlyContinue) {
    throw 'ゲームを終了してから実行してください。'
}

if ($Mode -eq 'Restore') {
    if ($targetState -eq 'NotFound') {
        Write-Output 'ボイスキープ用の patch2.xp3 はありません。変更は行いませんでした。'
        exit 0
    }
    if ($targetState -ne 'Patched') {
        throw 'patch2.xp3 はこのパッチが作成した内容と一致しないため、削除しませんでした。'
    }
    [System.IO.File]::Delete($targetPath)
    if ((Get-TargetState) -ne 'NotFound') {
        throw 'patch2.xp3 の削除後の確認に失敗しました。'
    }
    [pscustomobject]@{
        結果 = 'ボイスキープ用パッチを削除しました'
        対象 = $targetPath
        既存のpatch = (Join-Path $GameDirectory 'patch.xp3')
    }
    exit 0
}
...
if ($targetState -eq 'Unsupported') {
    throw '別の patch2.xp3 がすでに存在します。上書きは行いませんでした。'
}
...
if ($sourceState -ne 'Supported') {
    throw 'data.xp3 は未対応版または変更済みです。変更は行いませんでした。'
}

$tempPath = Join-Path $GameDirectory ('.voice-keep-' + [guid]::NewGuid().ToString('N') + '.tmp')
try {
    $resultHash = [VenusBloodVoiceKeepPatch]::Build($sourcePath, $tempPath)
...
[pscustomobject]@{
    ...
    作成 = $targetPath
    変更しないファイル = (Join-Path $GameDirectory 'patch.xp3')
}
