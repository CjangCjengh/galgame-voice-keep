...
game_id: "imouto-choukyou-nikki"
permalink: /games/imouto-choukyou-nikki/
---
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".\patch-voicecut.ps1" -GameDirectory "D:\Games\妹調教日記"
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".\patch-voicecut.ps1" -Mode Check -GameDirectory "D:\Games\妹調教日記"
```

## 元に戻す方法

初回適用時に `scene.int.before-voicecut-patch.bak` が作成されます。次のコマンドで元に戻せます。

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".\patch-voicecut.ps1" -Mode Restore -GameDirectory "D:\Games\妹調教日記"
```
