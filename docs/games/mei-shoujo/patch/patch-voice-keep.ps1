...
if ([string]::IsNullOrWhiteSpace($GameDirectory)) {
    if (Test-Path -LiteralPath (Join-Path $PSScriptRoot 'meishoujo.exe') -PathType Leaf) {
...
$targetPath = Join-Path $GameDirectory 'meishoujo.exe'
$expectedSize = 894464
$expectedHash = 'B4EBF89621391C7DB234913BF89F86C3458176BFD9003E23863C374A185D547A'
$patchRva = 0x3692F7
$originalBytes = [byte[]](0xE8, 0xE4, 0x02, 0xFF, 0xFF)
$patchedBytes = [byte[]](0x83, 0xC4, 0x04, 0x90, 0x90)
$localConfigPath = Join-Path $GameDirectory 'config.cfg'
$appDataConfigPath = Join-Path ([Environment]::GetFolderPath('ApplicationData')) 'Tanuki Soft\姪少女\config.cfg'

if (-not (Test-Path -LiteralPath $targetPath -PathType Leaf)) {
    throw "meishoujo.exe が見つかりません: $targetPath"
}
...
if ($target.Length -ne $expectedSize -or
    ...
    throw 'meishoujo.exe は未対応版または変更済みです。処理は行いませんでした。'
}

function Ensure-GameConfig {
    if ((Test-Path -LiteralPath $localConfigPath -PathType Leaf) -or
        (Test-Path -LiteralPath $appDataConfigPath -PathType Leaf)) {
        return
    }

    $configLines = @(
        '[archive]',
        'scenario="datascn.tac"',
        'pic="datapic.tac"',
        'voice="datavoc.tac"',
        'music="datamus.tac"',
        'se="datase.tac"',
        ...
        '[config]',
        'enable_d3d=true',
        'enable_no_resolution_change=true',
        '',
        '[setting]',
        'screen=window'
    )
    $configText = ($configLines -join "`r`n") + "`r`n"
    [System.IO.File]::WriteAllText($localConfigPath, $configText, [System.Text.Encoding]::ASCII)
    Write-Host "起動に必要な config.cfg を作成しました: $localConfigPath"
}
...
function Get-GameProcesses {
    return @(
        Get-Process -Name 'meishoujo' -ErrorAction SilentlyContinue | Where-Object {
            try {
...
if ($Mode -eq 'Check') {
    if ($processes.Count -eq 0) {
        $configState = if ((Test-Path -LiteralPath $localConfigPath -PathType Leaf) -or
            (Test-Path -LiteralPath $appDataConfigPath -PathType Leaf)) {
            'あり'
        } else {
            'なし（Launch時に作成）'
...
if ($processes.Count -ne 0) {
    throw '姪少女はすでに起動しています。終了してからLaunchモードを実行するか、Attachモードを使ってください。'
}
